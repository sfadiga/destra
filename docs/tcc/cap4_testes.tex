...
\begin{figure}[H]
    ...
    \caption{Osciloscópio digital FNIRSI® DSO-153 2-em-1. O equipamento é mostrado 
    durante medição de sinais do Arduino UNO, com a sonda conectada a um dos pinos 
    de debug. A tela colorida TFT exibe a forma de onda capturada em tempo real, 
    permitindo análise de frequência, jitter e tempo de processamento de comandos.}
...
Os testes realizados têm como objetivo avaliar o comportamento do protocolo DESTRA sob diferentes condições de operação, com foco em dois cenários: ferramenta e embarcado. Cada um desses cenários visa levantar as características de performance do protocolo, avaliando diferentes dimensões sob a perspectiva de testes.
...
\subsection{Cenário da Ferramenta}

Para o cenário da ferramenta foi desenvolvido um script Python (\texttt{performance\_tests.py}) complementar à ferramenta host que faz uso do protocolo DESTRA. O script realiza sequências de comando peek em uma variável inteira de tamanho 4 bytes.
...
A execução dos testes foi realizada conectando-se o Arduino ao host e utilizando o programa instrumentado no Arduino. O script Python foi executado através do comando:

\begin{verbatim}
> python .\performance_tests.py COM5
\end{verbatim}

Este comando irá executar a sequência dos três testes (Latência, Estresse e Rajada), coletando dados de performance internos entre cada teste. Porém é preciso ressaltar que a limitação de memória do Arduino permite guardar somente 100 registros de dados de performance. Logo para os testes mais longos como Estresse e Burst os dados internos são limitados a apenas 100 amostras.
A execução dos testes foi realizada conectando-se o Arduino ao host utilizando o programa 
instrumentado. O script Python foi executado através do seguinte comando:

\begin{verbatim}
> python .\performance_tests.py COM5
\end{verbatim}

Este comando executou automaticamente a sequência dos três testes (Latência, Estresse e 
Rajada), coletando dados de performance internos e gerando relatórios. A saída do console 
durante a execução é apresentada na Listagem \ref{lst:performance_tests_output}:
...
\subsubsection{Arquivos de Relatório Gerados}
...
\subsection{Testes com Osciloscópio}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth, angle=90]{recursos/figuras/osciloscopio_setup.jpg}
    \caption{Setup do osciloscópio conectado ao Arduino.}
