...
\subsection{Software Embarcado Crítico}
...
Apesar das diferenças entre os setores, um desafio comum permeia o desenvolvimento de software embarcado crítico: a necessidade de gerar evidências objetivas de segurança por meio de testes e campanhas de ensaios. Testes não apenas demonstram o atendimento aos requisitos definidos em projeto, mas também fornecem a documentação necessária a autoridades certificadoras para comprovar conformidade com normas de segurança. Em ambientes de alta criticidade, a ausência de testes rigorosos compromete não apenas o produto final, mas a viabilidade de sua certificação e entrada em operação.
...
Diversos meios físicos e protocolos são tradicionalmente empregados para esse fim, entre os quais se destacam UART, CAN e Ethernet. Cada um desses protocolos apresenta vantagens e limitações específicas, e normalmente sua escolha também está associada a escolha da arquitetura do sistema no qual o software irá executar, sendo assim uma decisão sistema e mais abrangente, é comum o protocolo estar associado a uma restrição física imposta por decisões no nível de hardware sejam elas derivadas de requisitos de custo ou requisitos funcionais. A caracteristica comum a todas os protocolos escolhidos é que sigam o paradigma de comunicação serial. A comunicação via UART é simples, de baixo custo e amplamente disponível, porém oferece taxas de transmissão relativamente limitadas e menor robustez contra erros. O CAN fornece elevada imunidade a ruído e mecanismos nativos de priorização de mensagens, sendo bastante apropriado para ambientes automotivos e aeroespaciais, mas possui capacidade de carga útil reduzida por quadro. O Ethernet apresenta alta velocidade e baixa latência, porém exige uma camada extra que garanta seu uso em sistemas embarcados de tempo real/críticos o que pode introduzir sobrecarga e maior custo de implementação em sistemas embarcados críticos.
...
No contexto das ferramentas contemporâneas, diversas arquiteturas implementam mecanismos equivalentes aos descritos. O PX4, por exemplo, utiliza o uORB como middleware de mensagens, disponibilizando mecanismos de depuração baseados em tópicos internos, embora não forneça acesso direto à memória como um peek/poke tradicional. Microcontroladores STM32 oferecem depuração via GDB Server incorporado, permitindo inspeção de registradores e variáveis durante pausa do processador, mas sem capacidade integrada de depuração não intrusiva em execução contínua. Já o Microchip Data Visualizer opera por meio de canais de dados instrumentados, fornecendo telemetria estruturada e escrita remota de parâmetros, aproximando-se de um sistema híbrido entre telemetria e controle \cite{ripley2016embedded}.

A inclusão desta seção é fundamental para contextualizar o núcleo técnico da ferramenta proposta neste trabalho. A compreensão dos conceitos de peek, poke e telemetria, bem como de suas limitações, riscos e implementações equivalentes na indústria, fornece o embasamento necessário para justificar o desenvolvimento de um protocolo personalizado de depuração. Além disso, conecta diretamente o escopo deste trabalho às práticas modernas de instrumentação embarcada e às exigências normativas relacionadas à confiabilidade e à verificação independente em sistemas críticos.


\section{Testes Automatizados e Testbeds}

A automação de testes em sistemas embarcados é um elemento essencial para garantir confiabilidade, repetibilidade e rastreabilidade no ciclo de desenvolvimento, especialmente em aplicações críticas. Para isso, a utilização de testbeds ou bancadas de teste estruturadas permite reproduzir cenários reais de operação, validar requisitos e identificar regressões de maneira sistemática e controlada. Um testbed típico inclui hardware de suporte, instrumentos de medição, controladores externos, ambientes de simulação e ferramentas capazes de gerar estímulos e coletar respostas do dispositivo sob teste (DUT). Em sistemas embarcados, essa infraestrutura geralmente envolve um testbench que interage diretamente com o firmware por meio de interfaces de comunicação, barramentos ou instrumentação específica.
...
A execução de testes automatizados por comandos remotos tem ganhado ampla adoção devido à capacidade de controlar o DUT sem intervenção humana direta. Nessa abordagem, o testbench envia comandos estruturados ao firmware, solicita respostas específicas ou aciona funções internas instrumentadas. Além de permitir maior repetibilidade, esse modelo habilita a criação de suites de teste extensas, execução contínua (CI/CD) e integração com frameworks modernos de validação. Protocolos remotos também possibilitam testes stress, injeção de falhas, medições de tempo e coleta automatizada de métricas internas.

Um elemento crítico para qualquer arquitetura de testes automatizados é a produção de logs estruturados. Registros formais, coerentes e com semântica definida permitem reconstruir o comportamento do sistema, identificar falhas, correlacionar eventos e validar critérios de aceitação. Diferentemente de logs textuais livres, formatos estruturados — como JSON, CBOR ou mensagens binárias — facilitam análise automatizada, integração com ferramentas de rastreamento e geração de relatórios. Em ambientes regulados, logs estruturados apoiam requisitos de traceability e verification evidence exigidos por auditorias.

No contexto de sistemas embarcados modernos, técnicas avançadas como HIL (Hardware-in-the-Loop), SIL (Software-in-the-Loop) e PIL (Processor-in-the-Loop) ampliam significativamente a capacidade de validação. Testes SIL simulam o firmware em ambiente nativo ou emulador, permitindo validações rápidas antes da integração com hardware real. Testes PIL executam o código no processador real ou simulado, assegurando correção funcional e temporal da implementação final. Finalmente, testes HIL permitem avaliar o sistema embarcado interagindo com modelos de planta, sensores simulados e atuadores virtuais, sendo amplamente utilizados na indústria aeronáutica, automotiva e de robótica para validar cenários complexos e dinâmicos com alta fidelidade.
